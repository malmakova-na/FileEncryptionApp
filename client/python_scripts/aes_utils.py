...
def decode_file(filename_r, filename_w, key_d):
    ...
    print('\nВремя дешифрования: {}с'.format(time.time()-start_time))
    sys.stdout.flush()
    fr.close()
    fw.close()
...
def encode_file_AEAD(filename_r, filename_w, key_e):
    start_time = time.time()
    nonce = generate_nonce().to_bytes(16, "big")
    MAC = bytes(0)
    fsize = os.path.getsize(filename_r)
    rsize = 0
    print('Шифрование '+filename_r)
    ...
    # Шифруем nonce
    c = encrypt_AES(nonce, key_e)
    c_block = bytes(c)
    fw.write(c_block)

    MAC = encrypt_AES(bitwise_xor_bytes(bytes(MAC), nonce), key_e)
    ...
    while block:
        ...
        if block:
            if len(block) < Nb:
                for i in range(Nb - len(block)):
                    block += b'\x00'
            block = bitwise_xor_bytes(block, nonce)
            MAC = encrypt_AES(bitwise_xor_bytes(bytes(MAC), block), key_e)
            c = encrypt_AES(block, key_e)
            c_block = bytes(c)
            fw.write(c_block)
    MAC_block = bytes(MAC)
...
def decode_file_AEAD(filename_r, filename_w, key_d):
    start_time = time.time()
    fsize = os.path.getsize(filename_r)
    rsize = 0
    print('Дешифрование '+filename_r)
    ...
    while block:
        block = fr.read(Nb)
        if eof - rsize <= 16 and len(block) > 0:
            MAC_comp = block
            if bytes(MAC_comp) != bytes(MAC):
                print("Файл не прошел аутентификацию!")
                fw.close()
        ...
        i += 1
        if i == 1:
            nonce = decrypt_AES(block, key_d)
            MAC = encrypt_AES(bitwise_xor_bytes(bytes(MAC), nonce), key_d)
            continue
        remain_time =  round((time.time()-start_time)/rsize*(fsize-rsize), 1)
        if i % 1000 == 0:
            if block:
        ...
        if block:
            if len(block) < Nb:
                for i in range(Nb - len(block)):
                    block += b'\x00'
            m = decrypt_AES(block, key_d)
            MAC = encrypt_AES(bitwise_xor_bytes(bytes(MAC), m), key_d)

            m = bitwise_xor_bytes(m, nonce)
            m_block = bytes(m)
